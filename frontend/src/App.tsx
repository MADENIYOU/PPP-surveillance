...
function AppShell() {
  ...
  // Indicateur de connexion : fraîcheur réelle du backend (meta.generated_at,
  // rafraîchi côté serveur à chaque appel). Échec → lastUpdate null → "HORS LIGNE".
  const { data: health } = useQuery({
    queryKey: ["health-live"],
    queryFn: () => apiClient.get<AqiCurrentResponse>("/aqi/current"),
    refetchInterval: 30_000,
  });
  const lastUpdate = health?.meta?.generated_at ?? null;

  return (
    <div className="flex h-screen overflow-hidden bg-gray-950 text-gray-100">
      <Sidebar />
      <div className="flex flex-1 flex-col overflow-hidden">
        <Header live={<LiveIndicator lastUpdate={lastUpdate} />} />
